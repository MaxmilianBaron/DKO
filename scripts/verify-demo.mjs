...
async function main() {
  ...
  try {
    ...
    await waitFor(cdp, `document.querySelector('[data-action="app-close"]')`, 'login close action');
    ...
    await waitFor(cdp, `document.body.innerText.includes('Hlavní nabídka') && document.querySelector('[data-route="work"]') && document.querySelector('[data-route="history"]') && document.querySelector('[data-route="settings"]')`, 'main menu');
    await click(cdp, '[data-route="work"]');
    ...
    await waitFor(cdp, `document.querySelector('[data-route="menu"][aria-label="Zpět"]')`, 'history back arrow');
    ...
    await click(cdp, '[data-jump="video"]');
    await waitFor(cdp, `document.body.innerText.includes('Uložená videa') && document.body.innerText.includes('V001')`, 'video documentation');
    await click(cdp, '[aria-label="Přehrát video"]');
    await click(cdp, '[data-action="modal-close"]');
    await click(cdp, '[data-jump="settings"]');
    await waitFor(cdp, `document.body.innerText.includes('Kontrola dat')`, 'technician settings menu');
    ...
    if (!addedTechnicianId) throw new Error('Technician settings could not add a technician.');
    ...
    await waitFor(cdp, `document.body.innerText.includes('Heslo technika není správné') && state.technicians.find(account=>account.id===${JSON.stringify(addedTechnicianId)})?.active`, 'wrong technician removal password rejection');
    ...
    if (!addedBuildingId) throw new Error('Technician settings could not add a building.');
    await click(cdp, `[data-building-remove="${addedBuildingId}"]`);
    await evaluate(cdp, `document.querySelector('#admin-confirm-password').value='spatne';`);
    await click(cdp, '[data-action="admin-remove-confirm"]');
    await waitFor(cdp, `document.body.innerText.includes('Heslo technika není správné') && state.buildings.find(building=>building.id===${JSON.stringify(addedBuildingId)})?.active`, 'wrong building removal password rejection');
    await evaluate(cdp, `document.querySelector('#admin-confirm-password').value='demo';`);
    await click(cdp, '[data-action="admin-remove-confirm"]');
    await click(cdp, '[data-action="modal-close"]');
    ...
    socket.close();
    console.log('DKO demo verification passed: technician flow, video documentation, photo markup, signature, complete 48-item PDF, photo sheets, unified technician settings, assets and edge-to-edge mobile mode.');
  } finally {
    socket?.close();
