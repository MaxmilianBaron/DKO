...
async function capture(cdp,name){await evaluate(cdp,'window.scrollTo(0,0)');const metrics=await cdp.send('Page.getLayoutMetrics');const size=metrics.cssContentSize;const shot=await cdp.send('Page.captureScreenshot',{format:'png',captureBeyondViewport:true,clip:{x:0,y:0,width:size.width,height:size.height,scale:1}});await writeFile(join(output,`${name}.png`),Buffer.from(shot.data,'base64'));}
