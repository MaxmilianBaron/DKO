...
async function runHidden(command,args){await new Promise(resolvePromise=>{const process=spawn(command,args,{stdio:'ignore',windowsHide:true});process.once('exit',resolvePromise);process.once('error',resolvePromise);});}
...
async function waitFor(cdp,selector){for(let i=0;i<900;i+=1){if(await evaluate(cdp,`Boolean(document.querySelector(${JSON.stringify(selector)}))`))return;if(i===300||i===600)await cdp.send('Page.reload',{ignoreCache:true});await delay(100);}throw new Error(`Timed out waiting for ${selector}`);}
...
async function capturePhone(cdp,name){const clip=await evaluate(cdp,`(()=>{const r=document.querySelector('.phone').getBoundingClientRect();return {x:r.x,y:r.y,width:r.width,height:r.height,scale:1}})()`);const shot=await cdp.send('Page.captureScreenshot',{format:'png',captureBeyondViewport:false,fromSurface:true,clip});await writeFile(join(output,`${name}.png`),Buffer.from(shot.data,'base64'));}
...
async function main(){
  ...
  try{
    ...
    await cdp.send('Page.enable');await cdp.send('Runtime.enable');await cdp.send('Page.navigate',{url});await waitFor(cdp,'[data-action="login-technician"]');
    await capture(cdp,'01-login');
    await click(cdp,'[data-jump="work"]');await capture(cdp,'02-technician-work');
    await click(cdp,'[data-jump="inspection"]');await waitFor(cdp,'[data-section="outside_information"]');await capture(cdp,'03-inspection-sections');
    ...
    await click(cdp,'[data-jump="photo"]');await click(cdp,'[data-route="photo-capture"]');await click(cdp,'[data-action="choose-photo"]');await waitForImages(cdp);await capture(cdp,'10-mobile-real-photo-library');
    await click(cdp,'[data-library-photo="0"]');await click(cdp,'[data-action="use-photo"]');await waitForImages(cdp);await draw(cdp,'#markup-canvas');await capture(cdp,'09-mobile-photo-markup');
    socket.close();console.log(`Captured DKO demo screenshots to ${output}`);
  }finally{await stopProcessTree(browser,profile);await delay(300);await rm(profile,{recursive:true,force:true}).catch(()=>{});}
}
